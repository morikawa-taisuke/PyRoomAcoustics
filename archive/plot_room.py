...
def plot_room(channel=1, distance=3, array_type="liner"):
	...
	if array_type == "liner":
		mic_coordinate = rec_util.set_mic_coordinate(center=mic_center, num_channels=num_channel,
		                                             distance=distance * 0.01)  # 線形 マイクの座標
	else:
		mic_coordinate = rec_util.set_circular_mic_coordinate(center=mic_center, num_channels=num_channel,
		                                                      radius=distance * 0.01 / 2, rotate=False)  # 円形 マイクの座標
...
def plot_room_2D(config_path):
	...
	room_dim = np.array(position['room_dim'])
	mic_position = np.array(position['mic'])
	speaker_position = np.array(position['speech'])
	noise_position = np.array(position['noise'])
	print(f"room[{room_dim.shape}]: {room_dim}")
	print(f"mic[{mic_position.shape}]: {mic_position}")
	print(f"speaker[{speaker_position.shape}]: {speaker_position}")
	print(f"noise[{noise_position.shape}]: {noise_position}")
	# fig = plt.figure()
	# ax = fig.add_subplot()

	fig = plt.figure(figsize=(8, 7.2))
	plt.scatter(mic_position[:, 0], mic_position[:, 1], label="Microphones", marker="d", s=200, edgecolors="b")  # mic
	# plt.scatter(mic_coordinate2[0], mic_coordinate2[1], label="45°",  marker="d", s=300)  # mic
	plt.scatter(speaker_position[:, 0], speaker_position[:, 1], label="Speaker", c="red", s=200)  # speaker
	plt.scatter(noise_position[:, 0], noise_position[:, 1], label="Noise", marker="x", c='green', s=200)  # noise
	plt.xlabel("X [m]", fontsize=20)  # 軸ラベル
	plt.ylabel("Y [m]", fontsize=20)  # 軸ラベル
...
if __name__ == "__main__":
	...
	#
	# mic_center = room_dim / 2  # アレイマイクの中心[x,y,z](m)
	# mic_coordinate = rec_util.set_mic_coordinate(center=mic_center, num_channels=num_mic,
	#                                              distance=mic_distance * 0.01)  # 線形アレイの場合
	...
	config_path = "/Users/a/Documents/sound_data/mix_data/VCTK_DEMAND_4ch_linear_10cm_1000cm_1000cm_1000cm/metadata.json"
	plot_room_2D(config_path)