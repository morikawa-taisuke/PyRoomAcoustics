...
def serch_reverb_sec(reverb_sec, channel=1, angle=np.pi):
	reverb = reverb_sec
	cnt = 0
	room_dim = np.r_[5.0, 5.0, 5.0]
	""" 音源の読み込み """
	# 修正: load_wav は (data, sr) を返すため、アンパックしてデータのみを取得する
	target_data, _ = rec_util.load_wav(f"C:/Users/kataoka-lab/Desktop/PyRoomAcoustics/src/mymodule/p231_001.wav")
	...
	mic_codinate = rec_util.get_mic_array(mic_config, mic_center)
	# print(mic_codinate)

	# --- 修正箇所: get_source_positions を使用 ---
	# doas = np.array([
	#     [np.pi / 2., np.pi / 2],
	#     [np.pi / 2., angle]
	# ])  # 音源の方向[仰角, 方位角](ラジアン)
	...
	# 目的音声 (方位角90度, 仰角0度, 距離0.5m)
	speech_config = {
		'horizon': 90,
		'elevate': 0,
		'distance': 0.5
	}
	# ノイズ (方位角:引数angleを度数変換, 仰角0度, 距離0.7m)
	noise_config = {
		'horizon': np.rad2deg(angle),
		'elevate': 0,
		'distance': 0.7
	}

	speech_pos_list = rec_util.get_source_positions(speech_config, mic_center)
	noise_pos_list = rec_util.get_source_positions(noise_config, mic_center)

	# リストを結合して (3, N) のnumpy配列に変換
	source_pos_list = speech_pos_list + noise_pos_list
	source_codinate = np.array(source_pos_list).T
	# ---------------------------------------------
...
def recoding2(wave_files, out_dir, snr, reverb_sec, reverb_par, channel=1, distance=0, is_split=False, angle=np.pi,
	...
	""" ファイル名の取得 """
	signal_name = my_func.get_fname(wave_files[0])[0]
	noise_name = my_func.get_fname(wave_files[1])[0]
	# print(f"signal_name:{signal_name}")
	# print(f"noise_name:{noise_name}")

	""" 音源の読み込み """
	target_data = rec_util.load_wav(wave_files[0])[0]
	noise_data = rec_util.load_wav(wave_files[1])[0]
	...
	# print(type(target_data), type(noise_data))
	...
	mic_coordinate = rec_util.get_mic_array(mic_config, mic_center)
	...
	source_pos_list = speech_pos_list + noise_pos_list
	source_coordinate = np.array(source_pos_list).T
	# ---------------------------------------------
	...
	result_mix = result_mix / np.max(np.abs(result_mix))
	result_reverb = result_reverb / np.max(np.abs(result_reverb))
	result_noise = result_noise / np.max(np.abs(result_noise))
	result_clean = result_clean / np.max(np.abs(result_clean))
	...
	if is_split:
		...
		""" チャンネルをまとめて保存 """
		""" noise_reverbration """
		mix_path = Path(f"{out_dir}/noise_reverb/{signal_name}_{noise_name}_{snr}db_{int(reverb_sec * 1000):04}msec_{angle_name}.wav")
		my_func.exists_dir(mix_path)
		# print(f"result_mix.shape:{result_mix.shape}")
		rec_util.save_wav(mix_path, result_mix.T)  # 保存

		""" reverbration_only """
		reverb_path = Path(f"{out_dir}/reverb_only/{signal_name}_{int(reverb_sec * 1000):04}msec_{angle_name}.wav")
		my_func.exists_dir(reverb_path)
		# print(f"result_reverb.shape:{result_reverb.shape}")               # 確認用
		rec_util.save_wav(reverb_path, result_reverb.T)  # 保存

		""" nosie_only """
		noise_path = Path(f"{out_dir}/noise_only/{signal_name}_{noise_name}_{snr}db_{angle_name}.wav")
		my_func.exists_dir(noise_path)
		# print(f"result_nosie.shape:{result_noise.shape}")               # 確認用
		rec_util.save_wav(noise_path, result_noise.T)  # 保存

		""" clean """
		clean_path = Path(f"{out_dir}/clean/{signal_name}_{noise_name}_{snr}db_{int(reverb_sec * 1000):04}msec_{angle_name}.wav")
		my_func.exists_dir(clean_path)
		# print(f"result_clean.shape:{result_clean.shape}")               # 確認用
		rec_util.save_wav(clean_path, result_clean.T)  # 保存
...
def process_recoding_thread(angle, angle_name, reverb_sec=5):
	...
	if not os.path.isfile(reverb_par_json):
		reverb_par = serch_reverb_sec(reverb_sec=reverb_sec, channel=ch, angle=angle)  # 任意の残響になるようなパラメータを求める
