...
def recoding2(wave_files, out_dir, snr, reverbe_sec, reverbe_par, channel=1, distance=0, is_split=False, angle=np.pi,
              angle_name: str = "None"):
    ...
    max_result_data = max(np.max(np.abs(result_mix)), np.max(np.abs(result_reverbe)), np.max(np.abs(result_noise)),
    ...
    if is_split:
        ...
        """ チャンネルをまとめて保存 """
        """ noise_reverberation """
        mix_path = f"{out_dir}/noise_reverbe/{signal_name}_{noise_name}_{snr}db_{int(reverbe_sec * 1000):04}msec_{angle_name}.wav"

        # print(f"result_mix.shape:{result_mix.shape}")
        rec_util.save_wave(result_mix, mix_path)  # 保存
        """ reverberation_only """
        reverbe_path = f"{out_dir}/reverbe_only/{signal_name}_{int(reverbe_sec * 1000):04}msec_{angle_name}.wav"
        ...
        rec_util.save_wave(result_noise, noise_path)  # 保存
        """ clean """
        clean_path = f"{out_dir}/clean/{signal_name}_{noise_name}_{snr}db_{int(reverbe_sec * 1000):04}msec_{angle_name}.wav"
        # print(f"result_clean.shape:{result_clean.shape}")               # 確認用
        rec_util.save_wave(result_clean, clean_path)  # 保存
